  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-200 py-8 px-4" 
           style={{
             backgroundImage: "url('https://images.unsplash.com/photo-1507738978512-35798112892c?fm=jpg&q=60&w=3000&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8N3x8ZGlnaXRhbCUyMGxpYnJhcnl8ZW58MHx8MHx8fDA%3D')",
        ...
        <div className="text-center mb-12 bg-white/85 backdrop-blur-xl rounded-3xl p-10 border border-white/90 shadow-2xl">
          <h1 className="text-5xl font-bold text-slate-800 mb-4 tracking-tight">Digital Library</h1>
          <p className="text-slate-600 text-xl font-medium opacity-90">Discover and explore educational resources</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-8 mb-10 border border-white/90">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="relative">
              <span className="absolute top-1/2 left-5 transform -translate-y-1/2 text-slate-500 text-lg z-10">🔍</span>
              <input
                type="text"
        ...
        {loading && (
          <div className="text-center py-20">
            ...
            <p className="text-slate-600 text-xl font-medium mt-4">Loading resources...</p>
          </div>
        )}
        ...
        {!loading && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {resources.length === 0 ? (
              ...
              resources.map((r) => (
                <div key={r._id} className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/90 transition-all duration-500 hover:shadow-3xl hover:scale-105 hover:bg-white/95 transform-gpu">
                  <div className="p-8">
                    {/* Header */}
                    ...
                    {r.type === "virtual_book" && r.content && (
                      <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 mt-6 border border-green-200/50">
                        <h4 className="text-lg font-bold text-green-800 mb-3">Book Preview:</h4>
                    ...
                    <div className="mt-6 pt-4 border-t border-slate-200/50 space-y-3">
                      {(r.type === "pdf" && r.fileUrl) && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center px-6 py-3 bg-red-100 hover:bg-red-200 text-red-700 font-semibold rounded-xl transition-all duration-300 transform hover:scale-105"
                        >
                          Open PDF
                        </a>
                      )}
        ...
        <div className="text-center py-12 mt-16">
          <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-8 border border-white/80 shadow-lg">
            <p className="text-slate-600 text-lg font-medium">Made with ❤️ for learning</p>
          </div>
        </div>
        </div> {/* Closes max-w-7xl mx-auto px-6 */}
      </div> {/* Closes min-h-screen bg-gradient */}
      <Chatbot />
    </>
  );
};
